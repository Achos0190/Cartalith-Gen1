...
1. **Visuals, asset tier** (`docs/BIOME_AND_VISUALS_PLAN.md`): B1 parchment + B3 procedural icons shipped in v0.050; B4 coastal wave lines shipped in v0.051. Remaining visual increments: **B2 texture splatting** (blocked on user approving a CC0 ground-texture pack — assemble a Poly Haven/ambientCG candidate set) and B3 sprite icons (same gate); optionally B4's vector spline-traced coastlines. The zero-asset procedural tier is now essentially complete — next non-gated work is the 16k tiling pipeline.
