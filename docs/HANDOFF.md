...
0. **Tectonic feature graph — ACTIVE workstream** (`docs/research/tectonic-feature-graph.md`, user-prioritised): T0 shear field + boundary-type matrix (v0.058), **T1** boundary polyline graph (v0.060), T2 orogenic multi-ridge uplift kernel (v0.061), and **T3** boundary-type features (v0.062: per-type signed profiles — trench+arc for O–C/O–O, fold-rippled collision belts with foreland basins, rift grabens — selected by the T0 matrix, crust-aware trench side; off ⇒ bit-identical) SHIPPED. Next: **T4** transform offsets (shear-dominant segments laterally displace ridge axes; linear transform valleys) → T5 tuning/archetypes.
