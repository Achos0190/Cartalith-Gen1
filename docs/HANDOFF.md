...
- Repo `achos0190/cartalith-gen1`. v0.048–0.056 work lives on branch **`claude/map-painting-ux-v048-acjted`** (draft PR #2); earlier work (≤v0.047) on `claude/weather-gravity-cartalith-c4u12t` / PR #1. Push to the session's work branch, never to `main`.
- Current engine file: **`elevation_foundation_v0.056.html`** (older `v0.036–0.055` kept, never edited in place — new version = new file).
- Headless suite: **201 assertions, all green**. Run before & after any engine change:
  ```bash
...
1. **Visuals asset tier**: **in-app ZIP/CSV importer + Style tab + icon sprites + sample pack SHIPPED in v0.056** (`docs/ASSET_PACK_FORMAT.md`). Remaining: **B2 texture splatting** (wire pack textures into `landColorCore` via `viz.splat`, the v0.057 plan — pending after the user's ocean-currents view request) and optionally dropping real CC0 art (`docs/research/asset-candidates.md`) into the sample-pack format.
...
## Engine capability summary (v0.037→v0.056)
